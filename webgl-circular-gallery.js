// WebGL Circular Gallery based on ReactBits component
// Using OGL library for WebGL rendering
...
class Title {
  constructor({ gl, plane, renderer, text, textColor = "#AFBB30", font = "bold 24px Inter" }) {
    autoBind(this);
    this.gl = gl;
    this.plane = plane;
    ...
    this.program = new OGL.Program(this.gl, {
      ...
      fragment: `
        ...
        void main() {
          ...
          // Apply green hover effect
          vec3 greenTint = vec3(0.686, 0.733, 0.188); // #AFBB30
          color.rgb = mix(color.rgb, color.rgb * greenTint * 1.3, uHover * 0.6);
...
class WebGLCircularGallery {
  ...
  addEventListeners() {
    ...
    this.container.addEventListener("mousemove", (e) => {
      // Calculate which media item is being hovered
      if (this.medias) {
        this.medias.forEach((media, index) => {
          const rect = this.container.getBoundingClientRect();
          const x = e.clientX - rect.left;
          const mediaX = (this.screen.width / 2) + media.plane.position.x * (this.screen.width / this.viewport.width);
          const mediaWidth = media.plane.scale.x * (this.screen.width / this.viewport.width);
          
          const isHover = x > mediaX - mediaWidth/2 && x < mediaX + mediaWidth/2;
...
// Export for use
window.WebGLCircularGallery = WebGLCircularGallery;
